...
interface User {
  ...
  name: string;
  email: string;
  password: string; // In a real-world case, passwords should be handled more securely
}
...
const LoginForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const users: User[] = await response.json(); // Use the User type here

      // Check if the provided email exists
      const user = users.find((u) => u.email === email);

      if (user && user.password === password) {
        login({ name: user.name, email: user.email, role: 'customer' });
        router.push('/profile');
      } else {
